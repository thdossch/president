...
\end{center}
We vermenigvuldigen met 100 om het resultaat als percentage uit te drukken. Dit is een goede maat aangezien we hieruit vaak ook kunnen afleiden dat als hij veel president is, hij waarschijnlijk ook vaak vice-president is, en minder scum of high-scum. Om te testen resetten we de ranks van de spelers na 100 episodes. TODO: episodes = 1 game?
...
Met $\alpha = 0.1$ en $\gamma = 0.75$ (gaven de beste resultaten in het onderzoek in appendix A), behaalt de Q-learning agent volgende resultaten. 
\begin{table}[h]
        \centering
        \begin{tabular}{|c|c|c|}
                \hline
...
\appendix
\section{\\De beste learning rate en discount factor vinden}
...
\end{figure}\\
Voor ons model kozen wij voor een $epsilon\_decay$ factor van 0.9999 omdat we merken dat na 100000 (TODO:eig niet, er is convergentie na 30k dus een epsilon van 0.9999 is goed genoeg) episodes het model zo goed als stabiel is. Met 0.9999 bereiken we de minimale epsilon na ongeveer 30000 episodes wat betekent dat we na 30\% van de episodes voornamelijk exploitatie hebben.\\\\
In onderstaande tabel hebben we een simulatie gedaan voor de $epsilon\_decay$ factoren die in bovenstaande grafiek zijn geïllustreerd, met $\alpha = 0.1$ en $\gamma = 0.75$. Dit zijn de waarden voor de parameters die in het onderzoek in appendix A (TODO: reference) de beste resultaten gaven.
\begin{table}[H]
        \centering
