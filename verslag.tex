...
\end{figure}\\
Voor ons model kozen wij voor een $epsilon\_decay$ factor van 0.9999 omdat we merken dat na 100000 (TODO:eig niet..) episodes het model zo goed als stabiel is. Met 0.9999 bereiken we de minimale epsilon na ongeveer 30000 episodes wat betekent dat we na 30\% van de episodes voornamelijk exploitatie hebben.\\\\
...
We zien dat zelfs voor een $epsilon\_decay$ factor van 0.99 de agent goed presteert. Ook voor $epsilon\_decay = 0.99999$, waarbij er nog veel exploratie is na 100000 episodes, is dit het geval. We kunnen dus besluiten dat de $\epsilon$ parameter een beperkte invloed heeft op de prestatie van de agent.\\\\
 Met deze parameters behaalt onze agent volgende resultaten: TODO (ook laten trainen tegen heuristiek en dan resultaten tegen random en heuristiek snelle fix).
\begin{table}[H]
        \centering
        \begin{tabular}{|c|c|c|}
                \hline
...
\end{center}
En we initialiseren de state-actie paren op 10 voor de acties op een state die de zorgen dat de speler uitspeelt, gebruikmakend van de rewardfunctie die hierboven beschreven is, behalen we ook een mooie W/L van 45\% tegen random spelers. We kunnen dit uitzetten op een grafiek naarmate hoeveel hij traint (figuur 6), waarop we zien dat er eigenlijk al vanaf 20 000 episodes niet veel meer verbeterd wordt.\\
\begin{figure}[h]
\centering
\includegraphics[scale=0.45]{images/extended_q_table_results.png}
\caption{Resultaten met $\gamma = 0.75$ en $\alpha = 0.1$ doorheen de tijd}
\end{figure}\\
