...
\begin{itemize}
	...
	\item Een iets complexere reward functie, gebaseerd op de kaarten in je hand voor en na je gelegd hebt, geeft veel slechtere resultaten: ongeveer 30\% W/L. Om dit toch te doen werken, moeten we misschien de state uitbreiden. TODO: leg complexere reward uit?
\end{itemize}
...
We zouden dus de state nog meer moeten veralgemenen...TODO thor :x
...
\indent als gepast: return -1.5 + skip\_penalty\\
\indent anders: return relative\_hand\_score + aantal\_kaarten\_gelegd*0.05\\

TODO: leg uit score van hand en skip penalty\\\\
We hebben deze functie niet in 1 keer gevonden, maar hebben veel verschillende uitgetest. Het valt duidelijk te merken dat dit een heel gefabriceerde en gedetailleerde functie is. De relatieve hand score is een reward die meer gebaseerd is op het korte termijn gedrag van de agent, het is dus interessant om te kijken hoe de discount factor $\gamma$ hier een rol in speelt.
\begin{figure}[h]
\centering
\includegraphics[scale=0.45]{images/gamma-iteratie-dqn.png}
\caption{Resultaten variëren met parameter $\gamma$}
\end{figure}\\
We zien inderdaad dat bij een hele hoge gamma, de agent het zelfs slechter gaat doen dan de random spelers waar hij tegen speelt. (TODO: waarom? hij skipt dan wss enkel) In deze context houden we de discount factor dus beter lager dan 0.9. Op 0.6 vinden we ook nog een uitschieter die een iets lagere gamma geeft, maar voor de meeste gevallen lijken de waarden onder 0.9 het wel goed te doen.\\\\
Merk op dat we met het construeren van een heel specifieke reward functie, maar met moeite een W/L percentage van 45\% behalen tegenover de random spelers. Dit is nog steeds redelijk goed tegen de random spelers, maar zeker nog niet even goed als een heuristieke speler. Daarbij is het gedetailleerd fabriceren van een reward functie misschien niet de beste manier van aanpak. Op een punt loopt dit dus op een dood einde, en moeten we misschien de agent meer zijn eigen beslissingen laten maken, in plaats van te blijven "gokken" welke kleine aanpassing aan de reward functie een beter resultaat zou geven. De agent volgt namelijk exact wat we aangeven in de reward functie, maar we hebben al vaak gemerkt dat een kleine redeneringsfout de agent kan leiden tot heel vreemde acties.
...
Een voorbeeld voor de eerder negatieve rewards is een die we bij het zoeken naar een goede rewardfunctie vaak hebben zien gebeuren. Stel de state bestaat ook hier uit $(rang, aantal, aantal\_kaarten\_in\_hand)$ en is (4,2,9) en de agent heeft alle 4 de jokers. Dit komt zelden voor, waardoor dat de Q-waarde nog een redelijk "hoge" waarde dicht bij 0 zal hebben. Dit betekent dat de agent het leggen van de 4 jokers in 1 zet als een goeie actie zal beschouwen. Dit is echter niet het geval want dan is hij zijn 4 jokers kwijt.\\\\
